...
However, our model only focuses on retrieval task, but still get information from the other 2 tasks (transfer learning). We can focus on the retrieval task only by setting the weight parameter when initializing the `NoToGo()` model class.
